...
def request(method, path, body=None):
    ...
    record(method + " " + path, response.code, parsed)
...
def api_check(item):
    if item == "bug01":
        ...
        expect(status == 200 and any(entry["id"] == first["id"] for entry in body) and all(entry["categoria"] == "FICCAO" for entry in body), "Filtro de categoria incorreto")
        ...
        expect(isinstance(entry.get("id"), int) and entry["id"] > 0 and entry["id"] != 999999, "ID nao gerado")
        ...
        expect(entry["nome"] == "Nome persistido" and persisted["nome"] == "Nome persistido", "Nome perdido")
    elif item == "bug11":
        ...
        for kind in ["filme", "serie", "documentario"]:
            ...
            expect(entry["id"] != 888888, "Cadastro de conteudo aceitou ID externo")
...
with (OUT / (args.label + ".log")).open("w", encoding="utf-8") as log:
    ...
    process = subprocess.Popen([str(JAVA_HOME / "bin" / "java.exe"), "-Dfile.encoding=UTF-8", *app, f"--server.port={port}", "--server.address=127.0.0.1", "--spring.datasource.url=jdbc:h2:mem:checkpoint_verificacao", "--spring.datasource.driverClassName=org.h2.Driver", "--spring.datasource.username=sa", "--spring.datasource.password=", "--spring.jpa.hibernate.ddl-auto=create-drop"], stdout=log, stderr=subprocess.STDOUT)
    try:
        deadline = time.monotonic() + 45
        while True:
            try:
                request("GET", "/api/conteudos")
                break
            except (OSError, urllib.error.URLError):
                if process.poll() is not None or time.monotonic() > deadline: raise RuntimeError("API nao iniciou; consulte " + str(log.name))
                time.sleep(0.25)
        for item in ITEMS:
